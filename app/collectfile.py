...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            print('python collectfile.py -e phobostilt -r "/srv/gwr/" -p scp -s GWRSG_12345_0002 -t SGO -b "2012-06-01" -d 30 -a "%y%m%d" -f "G1%s.025" -g -l "/srv/archive"')
            print('---------')
            ...
            for filename in filelist:
                ...
                if protocol == 'scp':
                    ...
                    if not user =='' and not address == '' and not password == '':
                        ...
                        scptransfer(user+'@'+address+':'+transferpath,tmppath,password,timeout=30)
                        #print "Data received ..."
                ...
                try:
                    ...
                    if not localpath == '':
                        ...
                        print("Writing data ({} points) to archive directory {}".format(data.length()[0],rawpath))
                        ...
                        if tmpname == '':
                            ...
                            for truefile in glob(tmpname):
                                ...
                                if zipping:
                                    ...
                                    os.remove(oldpath)
                                try:
                                    print ("Removing temporary file: {}".format(truefile) )
                                    os.remove(truefile) # cleanup tmp directory
                                    print (" ... done")
                                except:
                                    pass
                        else:
                            ...
                            if zipping:
                                ...
                                os.remove(oldpath)
                            try:
                                print ("Removing temporary file: {}".format(tmpname) )
                                os.remove(tmpname) # cleanup tmp directory
                                print (" ... done")
                            except:
                                pass
                    #if not tmppath == '':
