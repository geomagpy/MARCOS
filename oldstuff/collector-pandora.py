...
try:
    ...
    from collector import subscribe2client as cl
...
if __name__ == '__main__':
    ...
    scpuser = mpcred.lc('ceres','user')
    scppasswd = mpcred.lc('ceres','passwd')
    ...
    callfile = os.path.join(martaspath,'callsensors.txt')
    destsensfile = os.path.join(destpath,'MartasSensors',clientname+'_sensors.txt') 
    destcallfile = os.path.join(destpath,'MartasSensors',clientname+'_call.txt')
    ...
    try:
        ...
        sys.exit()
    print "Searching for sensors based on the call protocol"
    try:
        print scpuser+'@'+clientip+':'+callfile
        scptransfer(scpuser+'@'+clientip+':'+callfile,destcallfile,scppasswd)
    except:
        print "Call protocol obviously not used on client %s - proceeding" % clientname
        pass
    ...
    with open(destsensfile,'rb') as f:
        ...
        for line in reader:
            ...
            if len(line) < 2:
                ...
                s.append(line)
    print s
    if os.path.exists(destcallfile):
        sl = [line for line in s if line[0] == 'SERIAL']
        print "Serialline found", sl
        if not len(sl[0]) < 2:
            with open(destcallfile,'rb') as f:
                reader = csv.reader(f)
                ca = [[line[0],sl[0][1],sl[0][2]] for line in reader]
            print ca
            s.extend(ca)
            s = [elem for elem in s if not elem[0] == 'SERIAL']
        print "Sensor list after adding actively called sensors", s
